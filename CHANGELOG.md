...
### Added
- Added sample name validation for the config-driven runner to reject unsafe path and shell metacharacters.
- Added BWA index preflight validation for non-empty `.amb`, `.ann`, `.bwt`, `.pac`, and `.sa` sidecar files before `validate-config` and `run`.
- Added best-effort command-line tool version collection to `run_metadata.json` for reproducibility.

### Changed
- Removed an unreachable FASTA directory `st_mode` guard and moved `os` importing to module scope.
- Updated docs for v0.5.1 preflight and reproducibility behavior.

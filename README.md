...
`microC-pipeline` is a Micro-C preprocessing repository. The v0.5.x development line keeps the lightweight config-driven single-sample runner introduced in v0.4.0, makes valid pairs, matrix, stats, QC, and manifest files first-class validated products, and hardens preflight/reproducibility checks in v0.5.1.
...
The required genome FASTA must have non-empty BWA index sidecars (`.amb`, `.ann`, `.bwt`, `.pac`, and `.sa`) before `validate-config` or `run` succeeds. If `genome.chrom_sizes` is not provided, `genome.fasta` should also have an existing non-empty `.fai` index, or the FASTA directory must be writable so the runner can create the index with `samtools faidx`.
...
The v0.5.x standardized output naming and validation layer applies to the config-driven runner, not to the retained legacy root-level layout.

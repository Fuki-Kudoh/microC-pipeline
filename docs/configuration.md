...
| --- | --- | --- | --- |
| `sample` | Yes | None | Sample identifier used in the per-sample output directory and standardized output names. Must match `^[A-Za-z0-9][A-Za-z0-9._-]*$`; spaces, slashes, and shell metacharacters are rejected. |
...
| `genome.fasta` | Yes | None | Reference FASTA path. Non-empty BWA index sidecars (`.amb`, `.ann`, `.bwt`, `.pac`, `.sa`) must already exist for `validate-config` and `run`. |
...
| `bin_sizes` | No | `[1000]` | Positive integer bin sizes. v0.5.x uses the first value for `.cool`; optional `.mcool` is produced by `cooler zoomify`. |
...
Before running, `validate-config` and `run` also preflight-check sample naming, input file presence, chromosome-size feasibility, and the required BWA index sidecars.
